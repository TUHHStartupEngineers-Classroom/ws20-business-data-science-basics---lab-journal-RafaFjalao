...
tic()
 question <-combined_data[current_loan_delinquency_status >= 1, 
                           :N][
                             , max(current_loan_delinquency_status), by = loan_id][
                               order(V1, decreasing = TRUE)]
 
 curiosity<- combined_data[, max(current_loan_delinquency_status), by = loan_id][
   order(V1, decreasing = TRUE)]


combined_data[current_loan_delinquency_status > 3, .N, by = loan_id]
